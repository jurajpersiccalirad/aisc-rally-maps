...
export function EventNameInput() {
  ...
  useEffect(() => setLocalName(eventName), [eventName]);
  useEffect(() => setLocalVersion(version), [version]);
    ...
    <div className="space-y-2">
      <label className="block">
        ...
        <input
          ...
          onBlur={(e) => dispatch({ type: 'SET_EVENT_NAME', name: e.target.value })}
      ...
      <label className="block">
        ...
        <input
          ...
          onBlur={(e) => dispatch({ type: 'SET_VERSION', version: e.target.value })}
          className="mt-1 w-full rounded border border-slate-300 px-2 py-1 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-amber-400"
          placeholder="v1"
        />
        <span className="text-[11px] text-slate-500">
